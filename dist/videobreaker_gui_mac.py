...
class VideoBreakerGUI:
    ...
    def get_ffmpeg_path(self):
        ...
        # Try bundled FFmpeg first (no .exe extension on macOS)
    ...
    def setup_ui(self):
        ...
        ffmpeg_status = "✓ Bundled FFmpeg detected" if self.ffmpeg_path.endswith('ffmpeg') and os.path.exists(self.ffmpeg_path) and not self.ffmpeg_path == 'ffmpeg' else "⚠ Using system FFmpeg"
    ...
    def process_video(self):
        try:
            ...
            # macOS doesn't need special flags - subprocess runs silently by default
            result = subprocess.run(commands_list, capture_output=True, text=True)
